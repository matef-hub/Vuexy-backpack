<form role="form" method="POST" action="{{ route('backpack.auth.register') }}" class="mb-4">
  @csrf
  ...
  <div class="mb-6 form-control-validation">
    ...
    <input tabindex="2" type="{{ backpack_authentication_column()==backpack_email_column() ? 'email' : 'text' }}"
      ...
      name="{{ backpack_authentication_column() }}" id="{{ backpack_authentication_column() }}" value="{{ old(backpack_authentication_column()) }}">
  ...
  <div class="mb-6">
    <button tabindex="5" type="submit" class="btn btn-primary d-grid w-100">
